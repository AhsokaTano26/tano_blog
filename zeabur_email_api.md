...
API 密钥可以在 Zeabur 控制台的 Zeabur Email 管理页面创建和管理。
基础 URL
  ...
  "emails": [
    {
      ...
      "bcc": ["archive@example.com"],      // 可选
      "reply_to": ["support@yourdomain.com"], // 可选
      "subject": "个性化邮件 1",
      "html": "<p>你好，用户1！</p>",
      "text": "你好，用户1！",             // 可选
      "attachments": [],                   // 可选
...
job_type	string	筛选来源：direct（单封发送）、scheduled（定时发送）、batch（批量发送）
job_id	string	筛选任务 ID（定时邮件 ID 或批量任务 ID）
响应
...
字段	类型	说明
job_type	string	邮件来源：direct（单封发送）、scheduled（定时发送）、batch（批量发送）
job_id	string	任务 ID。单封发送时为空；定时发送为定时邮件 ID，批量发送为批量任务 ID
错误处理
...
各方案的每月发送配额、超额计费价格与计费方式，请参阅配额与超额计费。
每日配额限制
