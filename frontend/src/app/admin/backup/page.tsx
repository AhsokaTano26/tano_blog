...
const syncConfigKeys = [
  ...
  'sync_ssh_key_path', 'sync_remote_backup_dir', 'sync_remote_upload_dir', 'sync_bandwidth_kbps',
];
          ...
          {activeTab === 'sync' && (
            <div className="max-w-3xl space-y-6">
              ...
              <div className="pt-5 space-y-4" style={{ borderTop: '1px solid var(--glass-border)' }}>
                ...
                <div><label className="block text-sm mb-1.5" style={{ color: 'var(--text-secondary)' }}>主机 SSH 目标</label><input value={syncConfig.sync_ssh_target || ''} onChange={e => setSyncConfig({ ...syncConfig, sync_ssh_target: e.target.value })} placeholder="sync@primary.example.com" className={inputClass} style={inputStyle} /><p className="text-xs mt-1" style={{ color: 'var(--text-info)' }}>仅备服务器需要填写。</p></div>
